...
Completed, community-approved versions of some guides here are published to EDI or Zenodo. Publication for these guides happens from repository branches that are created from the community-approved content in `main` and named for the guide (currently `releases-guide-special-cases` and `releases-guide-eml-bp`). The `ediorg/data-package-best-practices` repository is linked to a Zenodo dataset series (under Greg's account), so releases from the repository, in any branch, will be automatically published with new DOIs there. Below we use the EML Best Practices document, with a branch name of `releases-guide-eml-bp` as an example. The workflow is:

1. Locally render the documents in the `main` branch. Doing this runs a Makefile that renders the PDF and Word format guides and moves them to the `guide-eml-bp/` folder.

        quarto render

2. If there is no release branch for the guide yet, create one:

        git checkout -b releases-guide-eml-bp
    

3. If there is already a release branch, check it out and merge in changes from `main`.

        git checkout releases-guide-eml-bp
        git merge main

4. Add and commit the rendered PDF and Word files for the guide you will be publishing.

        git add guide-eml-bp/guide-eml-bp_v4.0.pdf guide-eml-bp/guide-eml-bp_v4.0.docx

5. Remove all the files that are not going to be published, and rename the `_bookquarto.yml` file.

        git rm -rf guide-domain-specific guide-special-cases / # Directories first
                   archive .github img
        git rm *.qmd *.yml Makefile .gitignore                 # Other items

        # This file would be used to render the PDF and Word versions
