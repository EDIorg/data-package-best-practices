...
1. Render the documents in the `main` branch. Doing this runs a Makefile that renders the PDF and Word format guides and moves them to the `guide-eml-bp/` folder (or `guide-special-cases/` if rendering that document).
...
5. Remove all the files that are not going to be published.

        git rm -rf .                           # Remove everything
        git checkout HEAD -- guide-eml-bp/*    # Add files for publication back in

    If you need to replace anything you can use `git checkout main -- path/to/file`
...
7. On GitHub, prepare and publish a release, using the tag above, for the new document in the `releases-guide-eml-bp` branch. Once this 
