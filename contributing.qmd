...
Completed, community-approved versions of some guides here are published to EDI or Zenodo. Publication for these guides happens from repository branches that are created from the community-approved content in `main` and named using the repository's shorthand name for the guide (currently `guide-special-cases` and `guide-eml-bp`). The `ediorg/data-package-best-practices` repository is linked to a Zenodo dataset series (under Greg's account), so releases from the repository, in any branch, will be automatically published with new DOIs there. Below we use the EML Best Practices document, with a branch name of `releases-guide-eml-bp` as an example. The workflow is:
...
7. On GitHub, prepare and publish a release, using the tag above, for the new document in the `releases-guide-eml-bp` branch.
